...
public class GetAllControlesBD {
	...
	PreparedStatement selectAllAnu;
	/**
	...
	public GetAllControlesBD(Connection conn) throws SQLException{
		...
		query = "select * from atencioncontrol " +
				"WHERE estado='2';";
		selectAllAnu = conn.prepareStatement(query);
	}
	/**
	...
	public List getAllControlesAnu() {
    	...
    	try{
			ResultSet result = selectAllAnu.executeQuery();
    		while(result.next())
    		{
    			object = new Control();
    			object.setCliente(result.getString(1).trim());
    			...
    			object.setServicio(result.getString(7).trim());
    			object.setCosto(result.getString(8).trim());
    			object.setMotivo(result.getString(9).trim());
